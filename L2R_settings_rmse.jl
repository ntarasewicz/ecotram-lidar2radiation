# Run twice, once for each met tower near EcoTram (forest and wetland). Adjust *.txt files:
# met_forest.txt
# met_wetland.txt

# Adjust "swrf" so SWin obs match location. 

# Also provided location and SWin obs for US-NR1 tower. Adjust "image_height" to 25.0 to match sensor height, or run lower. 
...
function las2rad_settings(basefolder::String)
    ...
    par_in = Dict(
        ...
        # need to modify 'las2rad.jl' as described in 'l2r_mods.ipynb'

        # time step settings
        "t1"  => "28.03.2023 00:01:00", # start time stamp; format "dd.mm.yyyy HH:MM:SS"
        "t2"  => "28.03.2023 23:59:00", # end time stamp
        ...
        # location settings
        "time_zone"   => -7, # relative to UTC (e.g CET = UTC+1) 
        "coor_system" => "UTM 13 N", 
    ...
    dat_in = Dict(
        # chm
        "dsmf" => basefolder*"/l2r_input/forest_fen.laz",
        # dtm
        "dtmf" => basefolder*"/l2r_input/DTM_niwot_buffer.tif",
        # dem
        "demf" => basefolder*"/l2r_input/DEM_Niwot_50m.tif",
        ...
        # dbh
        "dbhf" => basefolder*"/l2r_input/forest_fen_treeinfo.txt",
        # ltc
        "ltcf" => basefolder*"/l2r_input/forest_fen_treefilter.laz",
        # swr
        "swrf" => basefolder*"/l2r_input/met_wetland_SWin_28032023.txt"     # EcoTram wetland met tower
        # "swrf" => basefolder*"/l2r_input/met_forest_SWin_28032023.txt"    # EcoTram forest met tower
        )
